...
export function EnvVarRow({ envVar, onUpdate, onDelete }: EnvVarRowProps) {
  ...
  const maskedValue = '•'.repeat(Math.min(envVar.value.length, 24));
  ...
  return (
    <div className="flex items-center gap-2 py-2 group">
      ...
      <span className="text-sm font-mono text-zinc-500 flex-1 min-w-0 truncate">
        {showValue ? envVar.value : maskedValue}
      </span>
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setShowValue(!showValue)}>
          {showValue ? <EyeOff className="h-3.5 w-3.5 text-zinc-400" /> : <Eye className="h-3.5 w-3.5 text-zinc-400" />}
        </Button>
