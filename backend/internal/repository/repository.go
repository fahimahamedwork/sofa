...
func (r *DomainRepo) FindAll(ctx context.Context) ([]model.Domain, error) {
        var domains []model.Domain
        if err := r.db.WithContext(ctx).Order("created_at DESC").Find(&domains).Error; err != nil {
                return nil, err
        }
        return domains, nil
}
...
func (r *DatabaseRepo) FindAll(ctx context.Context) ([]model.Database, error) {
        var databases []model.Database
        if err := r.db.WithContext(ctx).Order("created_at DESC").Find(&databases).Error; err != nil {
                return nil, err
        }
        return databases, nil
}
