...
func RegisterRoutes(
	...
	protected.Use(middleware.AuthRequired(cfg))
	{
		...
		apps := protected.Group("/apps")
		{
			...
			apps.POST("/:appId/deployments", deployHandler.CreateDeployment)

			// App env vars
			apps.GET("/:appId/env-vars", envVarHandler.ListEnvVars)
			apps.POST("/:appId/env-vars", envVarHandler.CreateEnvVar)
			apps.PUT("/:appId/env-vars", envVarHandler.UpdateEnvVar)

			// App domains
			apps.GET("/:appId/domains", domainHandler.ListDomains)
			apps.POST("/:appId/domains", domainHandler.AddDomain)

			// App databases
			apps.GET("/:appId/databases", dbHandler.ListDatabases)
			apps.POST("/:appId/databases", dbHandler.ProvisionDB)

			// App volumes
			apps.GET("/:appId/volumes", volumeHandler.ListVolumes)
			apps.POST("/:appId/volumes", volumeHandler.AttachVolume)
		}
		...
		deployments := protected.Group("/deployments")
		{
			...
			deployments.POST("/:appId/rollback/:id", deployHandler.Rollback)
